import json
import subprocess
from urllib.parse import quote_plus as url_param_encode
...
class Translate:
    ...
    INPUT_TOOLS_URI = 'https://inputtools.google.com/request'
    INPUT_TOOLS_ITC = {
    ...
    @staticmethod
    def get_token():
        process = subprocess.Popen(["gcloud", "auth", "application-default", "print-access-token"],
    ...
    async def get_languages(self, *, force_call=False, add_to_cache=True) -> list[dict[str, str | bool]]:
        ...
        async with aiohttp.ClientSession() as sess:
            async with sess.get(self.url + '/supportedLanguages', params=params, headers=headers) as resp:
    ...
    async def detect_language(self, text: str, *, raw=False) -> dict[str, str | int | float] | None:
        ...
        async with aiohttp.ClientSession() as sess:
            async with sess.post(self.url + ':detectLanguage', data=data, headers=headers) as resp:
                resp.raise_for_status()
                ...
                return result  # https://cloud.google.com/translate/docs/reference/rest/v3/DetectLanguageResponse
    ...
    async def input_tools(self, text: str, language: str, *,
        ...
        for p_name, p_value in params.items():
            ...
            url += p_name_final + url_param_encode(p_value).replace('+', '%20')
    ...
    async def translate(self, text: str, target_language: str, *, source_language: str = None, raw=False) -> dict:
        ...
        async with aiohttp.ClientSession() as sess:
            async with sess.post(self.url + ':translateText', data=data, headers=headers) as resp:
                resp.raise_for_status()
                ...
                result = {
                    'translated': js['translations'][0]['translatedText'],
                    'text': original_text,
                    'convertedText': text,
                    'targetLanguageCode': target_language,
                }
